...
6. Specify the rest of the configuration as well:
    ...
    - `NumberOfHipChatMessagesToImportIntoTeamsMessage`: You may be able to guess :). If it's greater than 1 then multiple HipChat messages will be imported into a single Teams message. You can use this to overcome Graph API throttling limitations. It seems that a safe general maximum is about 25 (suitable for rooms with many HTML bodied notifications too), with more HipChat messages the request will be too large (depends on how long messages usually are, that can vary a lot); go with lower if you want to be sure. If you still get an error among the lines of "413 (Request Entity Too Large)" then you can close the app, temporarily lower the value, run it again for a bit, then close and configure a higher value again (do note that the "X messages imported into the channel." is only displayed when the amount of messages can be divided by 50, so you may not get such messages for a while). It can even happen that a single message is too large, in which case you need to shorten it in the export package's JSON file. A good strategy is try the value 50 (or even 100), then lower it to 15 for a few seconds if you get an error. The Teams API rate limit is at about 1800 requests a day, so you'll only be able to import 1800 messages if you don't use this option before throttling kicks in. Importing into multiple teams (see the `HipChatRoomsToTeams` option) may increase this overall limit.
