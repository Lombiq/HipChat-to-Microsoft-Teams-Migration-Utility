...
6. Specify the rest of the configuration as well:
    ...
    - `UploadAttachments`: If set to `true` HipChat file attachments will be uploaded to the respective Teams channels. Be aware though that while the files will be accessible from under the Files tab they won't be attached to any messages, so won't appear in the message list. Set to `false` if you don't want attachments to be uploaded.
